...
class Window:
    ...
    def draw_car_platoons(self):
        for road in self.sim.roads:
            # Draw vehicles
            for car_platoon in road.car_platoons:
                self.draw_vehicle(car_platoon, road)

    def draw_signals(self):
        for signal in self.sim.traffic_signals:
            for i in range(len(signal.roads)):
                color = (0, 255, 0, 0) if signal.current_cycle[i] else (255, 0, 0, 0)
